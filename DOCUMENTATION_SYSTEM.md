...
| `11_Implementation` | Bắt đầu code từ đâu, task nào, khi nào coi là xong | Dev thực thi |
...
- **10_ADR/PENDING_DECISIONS.md** — *chỉ tham chiếu*: registry quyết định kỹ thuật chưa giải quyết, mỗi mục phải resolve bằng một ADR trước khi code khu vực liên quan (ARCHITECTURE_GOVERNANCE.md mục 4).

### 11_Implementation
Cầu nối từ Documentation Bible sang thực thi code — chia nhỏ `09_Roadmap/TECHNICAL_ROADMAP.md` thành sprint/task cụ thể, gắn điều kiện vào là các mục trong `10_ADR/PENDING_DECISIONS.md` đã resolve. Không chọn ngôn ngữ/framework, không viết code — đó là `07_Development` sau khi đã có ADR chọn công nghệ.
